...
void DatabaseServerPublisherBanner::DeleteRecords(
    ledger::DBTransaction* transaction,
    const std::string& publisher_key_query) {
  DCHECK(transaction);
  auto command = ledger::DBCommand::New();
  command->type = ledger::DBCommand::Type::RUN;
  command->command = base::StringPrintf(
      ...
      kTableName,
      publisher_key_query.c_str());

  transaction->commands.push_back(std::move(command));

  links_->DeleteRecords(transaction, publisher_key_query);
  amounts_->DeleteRecords(transaction, publisher_key_query);
}
